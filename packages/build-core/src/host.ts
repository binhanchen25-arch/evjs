...
export interface BuildHostParser {
  parseModule(
    source: string,
    options?: BuildHostParseOptions,
  ): BuildHostParsedModule;
  formatParseError(
    error: unknown,
    options?: BuildHostParseErrorOptions,
  ): string;
}
...
export interface BuildHostModules {
  resolve(
    specifier: string,
    options?: BuildHostResolveOptions,
  ): Promise<string | undefined>;
  load<TModule = unknown>(
    specifier: string,
    options?: BuildHostLoadOptions,
  ): Promise<TModule>;
...
export interface BuildHostBundler {
  ...
  dev?(
    plan: BuildPlan,
    callbacks: BuildHostDevCallbacks,
  ): Promise<BuildHostDevController>;
}
...
export interface BuildHostWatch {
  watch(
    ...
    callback: BuildHostWatchCallback,
  ): BuildHostWatchSubscription;
}

export type BuildHostWatchCallback = (
  changes: readonly BuildHostFileChange[],
) => void | Promise<void>;
...
export interface CapabilityGate {
  assertCapability(
    capability: keyof BuildHostCapabilities,
    requirement?: string,
  ): void;
}
