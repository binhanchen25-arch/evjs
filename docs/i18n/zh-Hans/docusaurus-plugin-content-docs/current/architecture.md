...
API。file-convention 应用还会从 `@evjs/ev/route`、`@evjs/ev/server-context` 和
`@evjs/ev/transport` 导入 curated authoring API；框架生成代码通过
`@evjs/ev/_internal/*` 解析 client/server runtime internals。`@evjs/client`
...
@evjs/plugin-qiankun
  可选的 qiankun master/slave 微前端桥接插件
```
...
| 角色 | 包 | 导入建议 |
|------|----|----------|
| 框架面 | `@evjs/ev` | config/build/plugin/deployment API 和能力组合使用 `@evjs/ev`；file-convention 应用源码使用 `@evjs/ev/route`、`@evjs/ev/server-context` 和 `@evjs/ev/transport`。 |
...
| 微前端插件 | `@evjs/plugin-qiankun` | 当应用明确参与 qiankun master/slave 拓扑时，在 `ev.config.ts` 中配置它。 |
...
普通 file-convention 应用只应从 `@evjs/ev` 及其语义化 authoring subpaths
...
不要把 `@evjs/ev/*` 做成 `@evjs/client` 或 `@evjs/server` 的镜像
re-export；每个 `@evjs/ev/*` subpath 都必须围绕 evjs 用户语义整理成 curated
API。
...
这类历史拆分包。构建 helper 从 `@evjs/ev/_internal/build` 导出，manifest
契约从 `@evjs/shared/manifest` 导出。

文档中的代码示例也遵循同一包边界：file-convention 应用示例从 `@evjs/ev`、
`@evjs/ev/route`、`@evjs/ev/server-context` 或 `@evjs/ev/transport` 导入；standalone
...
@evjs/ev generated-only runtime internals
  ...
  @evjs/ev/_internal/* 子路径承载，并由 @evjs/client 和 @evjs/server internals 支撑
...
@evjs/plugin-qiankun
  基于 @evjs/ev plugin hooks 分层实现的可选 qiankun master/slave 桥接插件
```

`@evjs/ev/_internal/build` 不 import bundler adapter。Bundler adapter 消费 `BuildPlan`，不会在 bundling 之后重新扫描源码来发现框架语义。
`@evjs/ev/_internal/build` 子路径只暴露 CLI 和 bundler adapter 需要的 tooling API。
...
router bootstrap。Generated bootstrap 通过 `@evjs/ev/_internal/client/*` 承载。
