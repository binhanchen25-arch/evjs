...
SSR/PPR 页面会向 plan 添加 server render entries。PPR 页面会生成 shell renderer。
evjs 0.2 中 partial prerendering 仍是 experimental：公开编写模型是 React
`Suspense`，当前兼容实现还可以为受限的 `Suspense` + 直接
`lazy(() => import(...))` 形态生成内部 region renderer。任意 Suspense boundary
的 runtime postponed/resume 尚未实现。有内部 region 时，框架服务端会在服务
page route 时解析这些 regions，因此浏览器首屏仍然只有一次 document 请求。PPR
支持两种 document delivery mode：
...
内部 PPR region 的 cache metadata 会进入 manifest：
