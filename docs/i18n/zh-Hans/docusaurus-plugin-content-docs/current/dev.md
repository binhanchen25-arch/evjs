...
7. CLI 核心通过 `@evjs/ev/internal/server/node` 自动启动 API 服务器。
8. 为派生出的框架运行时路径设置反向代理，例如 `/__evjs/fn`、`/__evjs/ppr`
   和 `/__evjs/rsc` → `localhost:3001`。

## API 服务器运行时

开发模式下，evjs 会通过一个小型 Node bootstrap 运行已构建的服务端 bundle，并调用生成专用的 `@evjs/ev/internal/server/node` 入口。生产环境中，应根据目标宿主环境选择合适的运行时包装来部署产出的 `{ fetch }` handler。
